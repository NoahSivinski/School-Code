 ...
 *
 *The debugged version of this program asks the user
 *for a temperature in fahrenheit and a temperature 
 *scale the user wants to convert to. Using the valid
 *temperature in farenheit this programs converts 
 *the temperature in farenheit to the specified temperature scale. 
 *
...
public class TemperatureConverter {
	...
	public static void main(String[] args) {
		...
		} else { //Create else statement to terminate program in case of invalid user input 
		...
		// Set relational operators to allow for execution proper execution of multi-branch if-else statements
		if (tempScale < 1 && tempScale > 4) { // Second-if statement: Set a breakpoint here
			...
			if (tempScale == 1) { // Set a breakpoint here
				...
				tempScaleStr = "Kelvin";
			} else if (tempScale == 2) { //Remove boolean expressions that proves true for multiple branches of if-else 
				convertedDegrees = fahrenheit - MIN_FAHRENHEIT;
				tempScaleStr = "Rankine";
				//Add parentheses to aid in proper arithmetic execution 
			} else if (tempScale == 3) { 
				convertedDegrees = (fahrenheit - 32) * 4 / 9; // Set a breakpoint here
				tempScaleStr = "Reaumur";//Set variable equal to correct string
			} else {    //Properly incorporate conversion equation
