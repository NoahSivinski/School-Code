...
def test_code():
    ...
    result = data.loc[:]['temperaturemin'].mean()
    print(f"{result:.4f}")
    ...
    result = data.loc[:]['temperaturemax'].mean()
    print(f"{result:.4f}")
    ...
    result = pd.DataFrame({'temperaturemax': data.iloc[[3,5], 2], 'precipitation': data.iloc[[3,5], 3]})
    print(f"{result}")

    dotspace()
    print("  Task 2 b: ---- mean each column    : ")
    result1 = result.temperaturemax.mean()
    result2 = result.precipitation.mean()
    ...
    #print(data.head(4)) # no difference until you add eval()
    data.eval('tempdiff = temperaturemax - temperaturemin', inplace = True)
    print(data.head(4))
    # Task 4. :
    #
    dotspace()
    ...
    filter_value = np.where((data.temperaturemin >= 50) & (data.temperaturemax <= 75))
    #filter_value = data.where(data.temperaturemax <= 75)
