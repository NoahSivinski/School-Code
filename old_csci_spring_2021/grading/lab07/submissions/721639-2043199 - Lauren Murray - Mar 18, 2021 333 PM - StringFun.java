 ...
 *
 * Purpose:This program uses six possible commands to manipulate Strings that the 
 * user inputs. The commands include: Reverse, Replace First, Replace Last, Remove,
 * Remove All, and Quit. 
 *
...
public class StringFun {
	...
	public static void main(String[] args) {
		...
		do {
			System.out.println("Enter your command (reverse, replace first, replace"
					+ "last, remove all, remove, quit) ");
			...
			if (!command.equalsIgnoreCase("reverse") && !command.equalsIgnoreCase("replace first") &&
				...
				System.out.println("Please enter one of the commands (reverse, replace first,"
			...
			if (command.equalsIgnoreCase("replace first")) {
				...
				if (manipulated.contentEquals(inputedString)) {
			...
			if (command.equalsIgnoreCase("replace last")) {
				...
				int indexOfSearch = -1;
						...
						if (manipulationLast.contentEquals(inputedString)) {
							System.out.println("The letter was not found in the word");
						}
						else {
			...
			//remove a specific instance of a character in the String
			if (command.equalsIgnoreCase("remove")) {
				System.out.println("Enter the character to remove");
				remove = keyboard.next().charAt(0);
				keyboard.nextLine();
				System.out.println("Enter the " + remove + " you would like to remove (Not the index"
						+ " - 1 = 1st, 2 = 2nd, etc.)");
