 ...
 *
 * Purpose: This lab allows you to visually represent large numbers 
 * of classes and other structures as well as how they interrelate.
 * It stores an array of double values called data. This is
 * a modified version that will allow you to find the min, max,
 * mode, average of the values, along with some new feautures.
 * This will now allow a data array of 0 elements. This lab will
 * also now be able to handle methods invoked with null as a parameter.
 * It will also compute the variance and standard deviation of the 
 * stored data values.
 * 
 *
...
public class Stat {
	...
	// creates a double array of the same length as d 
	...
	// creates a new array containing the elements of d and assign to
	...
	public void setData(double[] d) {
		try {
			...
			this.data = new double[0];// if the entered value is null it will create an empty array
		}
	}
	...
	public boolean equals(Stat s) {
		...
		try {
			...
			return false;// if the entered value is null then it will return false
		}
	}
	...
	public void append(double[] d) {
		try {
			...
			// if the entered value is null, nothing will happen
		}
	}
	...
	public double max() {
		if (this.data.length > 0) {
			...
			return Double.NaN;// if length is zero then it will return not a Number
		}
		return Double.NaN;// if length is zero then return not a Number
	}
	...
	public double average() {
		...
		return s / this.data.length;// if zero not a Number
	}
	...
	public double mode() {
		if (this.data.length > 0) {
			...
			return Double.NaN;// if length is zero then not a Number
		}
		return Double.NaN;// if length is zero then not a Number
	}
