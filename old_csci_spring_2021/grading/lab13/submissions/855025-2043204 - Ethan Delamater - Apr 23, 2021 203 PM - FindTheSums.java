 ...
 *
 * Purpose: Creates a virtual word search puzzle through 2D arrays. 
 * Instead of words this uses integers and sums.
 *
...
public class FindTheSums {
	...
	public static int[][] horizontalSums(int[][] a, int sumToFind) {
		...
		int[][] b = new int[a.length][a[0].length]; // create the b array
	...
	public static int[][] verticalSums(int[][] a, int sumToFind) {
		...
		int[][] b = new int[a.length][a[0].length]; // recreating the b array
		for (int i = 0; i < b.length; i++)
			for (int j = 0; j < b[0].length; j++)
				b[i][j] = 0;
		...
		for (int i = 0; i < a[0].length; i++) { // columns
			for (int j = 0; j < a.length; j++) { // rows
				...
				if (sum == sumToFind) { // updating b array
					for (int k = 0; k < tracker; k++)
						b[j + k][i] = a[j + k][i];
				}
			}
