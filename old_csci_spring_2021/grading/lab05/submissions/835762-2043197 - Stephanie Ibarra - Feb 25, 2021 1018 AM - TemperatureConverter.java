...
public class TemperatureConverter{
	...
	public static void main(String[] args) {
		...
		if (fahrenheit > MIN_FAHRENHEIT) {   // first if-statement: Set a breakpoint here
			System.out.print("The temperature is greater than or equal to " + MIN_FAHRENHEIT);
		...
		if (tempScale < 1 && tempScale > 4) {  // Second-if statement: Set a breakpoint here
			System.out.println("Unknown temperature scale  -" +	// Change the limits so that only 1-4 are accepted
					"  cannot do calculation. Bye");
		}
		else {
			if (tempScale == 1) { // Set a breakpoint here
				convertedDegrees = (fahrenheit + MIN_FAHRENHEIT)*5/9; // add () so that it's in the proper order to multiply
				tempScaleStr="Kelvin"; 
			} else if (tempScale == 2) { 
				convertedDegrees = fahrenheit + MIN_FAHRENHEIT;  // switch the - to +
				tempScaleStr="Rankine";

			} else if (tempScale == 3) { 
				convertedDegrees = (fahrenheit - 32)*4/9 ; // Set a breakpoint here. And added the ()
				tempScaleStr="Reaumur";// Changed it to Reaumur so that it matches up with the 4 choices and the function 

			} else {
				convertedDegrees = (fahrenheit - 32)*5/9; // Set a breakpoint here
				tempScaleStr="Celsius"; //Also switch the order of the 5 and 9 so it matches the correct equation
			}
