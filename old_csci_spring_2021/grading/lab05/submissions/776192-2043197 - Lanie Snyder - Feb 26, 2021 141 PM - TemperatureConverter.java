 ...
 *temperature scale the user wants to convert it to. If both the temperature
 *in Fahrenheit and the temperature scale are valid, the program converts
 *the temperature in Fahrenheit to the specified temperature scale and
...
public class TemperatureConverter {
	...
	public static void main(String[] args) {
		...
		if (fahrenheit < MIN_FAHRENHEIT) { // first if-statement
		...
		if (tempScale < 1 || tempScale > 4) { // Second-if statement
			System.out.println("Unknown temperature scale  -" + "  cannot do calculation. Bye");
		} else {
			if (tempScale == 1) { // Set a breakpoint here
				convertedDegrees = (fahrenheit - MIN_FAHRENHEIT) * 5 / 9; // minus because MIN_FAHRENHEIT is negative
				tempScaleStr = "Kelvin";
			} else if (tempScale == 2) {
				convertedDegrees = fahrenheit - MIN_FAHRENHEIT; // minus because MIN_FAHRENHEIT is negative
