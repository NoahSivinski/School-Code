 ...
 *
 * Purpose: This program is given values so that they can be tested.
 * 
...
public class Circle {
	...
	{
	if (((radius - 1.0)<THRESHOLD) && ((x-0.0)< THRESHOLD) && ((y-0.0)<
	THRESHOLD)) {
	return true;
	}
	else
	...
	{
	if ((anotherCircle.getRadius() + radius) >

	this.distance(anotherCircle)) {
	return true;
	}
	else {
	return false;
